...
*** Variables ***
${host}  http://test-eauction.brizol.net
...
Авторизація
    [Arguments]  ${username}
    Click Element  xpath=//*[contains(@href, "/login")][contains(text(), "Увiйти")]
...
Додати актив до об'єкта МП
    ...
    brizol.Пошук об’єкта МП по ідентифікатору  ${username}  ${tender_uaid}
    ...
    Run Keyword And Ignore Error  brizol.Додати предмет МП  ${item_data}
    Run Keyword And Ignore Error  brizol.Scroll To And Click Element   id=btn-submit-form
    Wait Until Element Is Visible  xpath=//div[@data-test-id="tenderID"]
...
Внести зміни в об'єкт МП
    ...
    brizol.Пошук об’єкта МП по ідентифікатору  ${tender_owner}  ${tender_uaid}
    Click Element  xpath=//a[contains(@href, "asset/update")]
    Wait Until Element Is Visible  xpath=//form[@id="asset-form"]
...
Завантажити документ для видалення об'єкта МП
    ...
    brizol.Завантажити документ в об'єкт МП з типом  ${username}  ${tender_uaid}  ${file_path}  cancellationDetails


Завантажити ілюстрацію в об'єкт МП
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}
  brizol.Завантажити документ в об'єкт МП з типом  ${username}  ${tender_uaid}  ${filepath}  illustration
...
Створити лот
  ...
  brizol.Пошук об’єкта МП по ідентифікатору  ${username}  ${asset_uaid}
...
Заповнити дані для першого аукціону
  ...
  brizol.Пошук лоту по ідентифікатору  ${username}  ${tender_uaid}
...
Завантажити ілюстрацію в лот
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}
  brizol.Завантажити документ в лот з типом  ${username}  ${tender_uaid}  ${filepath}  illustration
...
Завантажити документ для видалення лоту
  [Arguments]  ${username}  ${tender_uaid}  ${file_path}
  brizol.Завантажити документ в лот з типом  ${username}  ${tender_uaid}  ${filepath}  cancellationDetails
...
Завантажити документ в ставку
    ...
    brizol.Скасувати цінову пропозицію  ${username}  ${tender_uaid}
...
Отримати посилання на аукціон для глядача
    ...
    brizol.Пошук Тендера По Ідентифікатору  ${viewer}  ${tender_uaid}
